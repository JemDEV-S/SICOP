...
export async function POST(request: Request) {
  ...
  if (failed >= 5) return Response.redirect(absoluteUrl("/login?error=1", request));
  ...
  if (!ok || !user) return Response.redirect(absoluteUrl("/login?error=1", request));
  ...
  return Response.redirect(absoluteUrl("/admin", request));
}
